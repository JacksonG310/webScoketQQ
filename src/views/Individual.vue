<template>
  <div id="individual">
    ...
    <div class="main">
      <cell
        title="头像"
        :isLink="info.id == curUser"
        @click.native="toReset(info.profile)"
      >
  ...
  methods: {
    ...
    async handleConfirm(newValue, index) {
      ...
      if (this.type == "sex") {
        ...
        const { data: res } = await this.$http.put("/individual/updateSex", {
          sex: newValue,
          id: this.curUser,
        });
        if (res.code === 20000) {
        ...
        const { data: res } = await this.$http.put("/individual/updateBirth", {
          birth: newBirth,
          id: this.curUser,
        });
        if (res.code === 20000) {
    ...
    async updateSign() {
      ...
      const { data: res } = await this.$http.put("/individual/updateSign", {
        sign: this.sign,
        id: this.curUser,
      });
      if (res.code === 20000) {
    ...
    async getUserDetail() {
      ...
      if (res.code == 20000) {
        this.info = res.data;
        this.info.createdAt = timeFormat(this.info.createdAt);
      } else {
        Toast.fail("获取用户信息失败");
      }
    },
    ...
    toReset(profile) {
      this.$router.push({
        path: "/reset",
        query: { profile, id: this.curUser, username: this.info.name },
      });
    },
  },
  async created() {
