  ...
  methods: {
    ...
    onSearch: debounce(async function () {
      ...
      const { data: res } = await this.$http.get(
        `search/user?keyword=${this.keyword}`
      );
      if (res.code === 20000) {
        ...
        this.userResult.forEach((res) => {
          ...
          if (res.name.search(this.keyword) != -1)
            res.name = res.name.replace(replaceReg, replaceString);
          if (res.email.search(this.keyword) != -1)
            res.email = res.email.replace(replaceReg, replaceString);
        });
      } else {
    ...
    async judgeFriend(result) {
      const { data: res } = await this.$http.get(
        `/search/isFriend?uid=${this.userID}&fid=${result.id}`
      );
      if (res.code != 50000) {
    ...
    moreDetail(id, isFriend) {
      this.$router.push({
        path: "/friendDetail",
        query: { friendID: id, isFriend: isFriend ? 1 : 0 },
      });
    },
    onCancle() {
      Toast.fail("取消搜素");
